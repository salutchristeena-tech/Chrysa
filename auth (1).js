...
(function () {
  ...
  function close() {
    ...
    setTimeout(() => { if (scrim) { scrim.remove(); scrim = null; } maybeGate(); }, 280);
  }
  ...
  function maybeGate() {
    if (scrim) return;
    if (!document.body || !document.body.hasAttribute("data-require-auth")) return;
    if (hasSession()) { document.body.classList.add("marc-unlocked"); return; }
    document.body.classList.remove("marc-unlocked");
    open({ gate: true, onEnter: () => { document.body.classList.add("marc-unlocked"); close(); } });
  }
  ...
  window.MarcAuth = { open, close, hasSession, signOut, exportCSV };
})();
